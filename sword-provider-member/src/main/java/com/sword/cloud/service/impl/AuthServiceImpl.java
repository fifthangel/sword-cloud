...
import com.sword.cloud.dao.PermissionMapper;
import com.sword.cloud.dao.RoleMapper;
import com.sword.cloud.dao.RolePermissionMapper;
...
import com.sword.cloud.service.AuthService;
...
public class AuthServiceImpl implements AuthService {
    ...
    public int addPermission(Permission permission) {
        return this.permissionMapper.insertSelective(permission);
    }
    ...
    public int updatePerm(Permission permission) {
        return this.permissionMapper.updateSelective(permission);
    }
    ...
    public String delPermission(int id) {
        ...
        if (this.permissionMapper.deleteById(Long.valueOf(id)) > 0) {
            return "ok";
        } else {
            return "删除失败，请您稍后再试";
        }
    ...
    public  List<Permission> findPerms() {
        List<Permission> perms = this.permissionMapper.findPerms();
        return  perms;
    }
    ...
    @Override
    public RoleView findRoleAndPerms(Integer id) {
    ...
    public String updateRole(Role role, String permIds) {
        ...
        log.debug("权限id =arrays=" + arrays.toString());
        //1，更新角色表数据；
        int num = this.roleMapper.updateSelective(role);
        if (num < 1) {
            //事务回滚
    ...
    public String delRole(int id) {
        ...
        //2.删除角色
        int num = this.roleMapper.deleteById(Long.valueOf(id));
        if (num < 1) {
            //事务回滚
            TransactionAspectSupport.currentTransactionStatus().setRollbackOnly();
            return "操作失败";
    ...
    private void batchDelRolePerms(int roleId) {
        ...
        if (null != rpks && rpks.size() > 0) {
            for (RolePermission rpk : rpks) {
                this.rolePermissionMapper.deleteByEntity(rpk);
            }
        }
    }
    ...
    private void setRolePerms(int roleId, String[] arrays) {
        for (String permid : arrays) {
            ...
            this.rolePermissionMapper.insertSelective(rpk);
        }
    }
}
